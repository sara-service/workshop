...
<xsl:stylesheet
	...
	<xsl:template name="itemSummaryView-DIM-type">
		<xsl:if test="dim:field[@element='type'][not(@qualifier) and descendant::text()]">
			...
			<p style="color:#000; text-align:left; padding: 5px; background-color:#c3cdd7; display:inline-block; border:#eee 1px solid">
                <xsl:for-each select="dim:field[@mdschema='dc'][@element='type'][not(@qualifier)]">
                    <xsl:value-of select="."/>
					<xsl:if test="count(following-sibling::dim:field[@mdschema='dc'][@element='type'][not(@qualifier)]) != 0">
                        <br/>
                    </xsl:if>
                </xsl:for-each>
			</p>
    ...
    <xsl:template name="itemSummaryView-DIM-thumbnail">
        <div class="thumbnail">
            <xsl:choose>
                ...
                <xsl:otherwise>
                    <img alt="[Standardicon]" style="border: 0px" width="66%" src="{concat($theme-path, 'images/Logo_SARA_RGB.png')}"/>
                </xsl:otherwise>
            </xsl:choose>
        </div>
    </xsl:template>
    ...
    <xsl:template name="itemSummaryView-DIM-date-created">
        <xsl:if test="dim:field[@element='date' and @qualifier='created' and descendant::text()]">
